...
class handDetector():
    ...
    def findHands(self, img, draw=True):    # Finds all hands in a frame
        if img is None:
            print("Warning: Received empty image for processing.")
            return img
    ...
    def findPosition(self, img, handNo=0, draw=True):   # Fetches the position of hands
        ...
        # Make sure that results exist and there are hand landmarks before proceeding
...
def main():
    pTime = 0
    cap = cv2.VideoCapture(0)  # Use the correct camera index (0 for default camera)
    ...
    while True:
        ...
        if len(lmList) != 0:
            print(lmList[4])  # Example of printing the position of landmark 4
